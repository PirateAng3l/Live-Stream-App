...
export async function createFixtureAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  // Blank (not just missing) whenever the Clean Slate/Event form didn't
  // render the away-team select at all — a prize-giving or concert has no
  // opposing team. Migration 0015's check constraint is the real backstop
  // (away_team_id is only allowed to be null when sport = 'other').
