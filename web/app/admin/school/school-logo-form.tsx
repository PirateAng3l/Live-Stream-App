...
import { updateSchoolLogoAction, type ActionState } from "./actions";

const initialState: ActionState = {};

export function SchoolLogoForm({ school }: { school: SchoolDetail }) {
  const [state, formAction] = useFormState(updateSchoolLogoAction, initialState);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);

  return (
    <div className="mx-auto max-w-sm">
      <h1 className="mb-1 text-2xl font-bold">School profile</h1>
      <p className="mb-6 text-sm text-textsecondary">{school.name}</p>
      ...
      <div className="mb-6 flex items-center gap-4">
        <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-lg border border-white/10 bg-panel">
          {previewUrl || school.logoUrl ? (
            ...
            <img src={previewUrl ?? school.logoUrl ?? ""} alt="" className="h-full w-full object-contain" />
          ) : (
            <span className="text-xs text-textsecondary">No logo</span>
          )}
        </div>
        <p className="text-xs text-textsecondary">
          Shown in place of the flat home-team colour block on the live stream overlay. PNG, JPEG,
          or WebP, up to 5MB.
        </p>
      </div>
